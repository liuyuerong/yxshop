<template>
  <div>
    ...
    <div id="login">
      ...
      <!-- 密码 -->
      <van-field placeholder="请输入密码" class="inp"   v-model="pwd"
    ...
    @click-right-icon="hidePwd = !hidePwd"
      ...
      :rules="[{ required: true, message: '不能为空' }]"
       ...
       />
       <!-- 确定密码 -->
      <van-field placeholder="确定密码" class="inp" v-model="confirm_pwd" 
    ...
    @click-right-icon="hidePwd1 = !hidePwd1"
      ...
      <!-- 城市 -->
      <van-field placeholder="城市" 
      v-model="area"
       class="inp" right-icon="wap-home" 
      @click="areaShow=true"
       />
      <!-- 城市李彪 -->
      <van-popup v-model="areaShow" position="bottom">
            <van-area title="选择城市" :area-list="areaList"
      ...
      <van-field placeholder="手机验证码" v-model="code" class="inp" >
        <template #button>
          <van-button type="danger" placeholder="请输入" plain size="small" :disabled="btnIsDisabled" @click="djs">{{msg}}</van-button>
        </template>
      </van-field>
...
checkForm(){
    // 验证不能为空
    if(this.mobile == "" ||this.pwd=="" || this.nick == "" ||this.picCode == ""||this.code=="" ){
        // 轻提示
...
#login {
  ...
  .btn {
    // linear-gradient()
    background-image: linear-gradient(to right, orange, #6149f6);
    border: none;
    box-shadow: 0px 4px 3px rgba($color: #000000, $alpha: 0.4);
  }
  .van-image{
      width: 3rem;
  }
